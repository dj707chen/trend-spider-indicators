// This script had been cloned from "B-XTrender" at 25 Nov 2024, 00:43
...
//
describe_indicator('Weekly B-XTrender - DO NOT USE FOR BACKTESTING THIS IS FORWARD LOOKING','lower');
...
const priceData = await request.history(current.ticker, 'W',{ chart_type: "candles", ext_session: false });

let shortTermXtrender = sub(rsi(sub(ema(priceData.close, short_l1), ema(priceData.close, short_l2)), short_l3),50);
...
let longTermXtrender  = sub(rsi(ema(priceData.close, long_l1), long_l2 ),50);
...
//Land and interpolate on current timeframe
const longTermXtrender_interp = interpolate_sparse_series(land_points_onto_series(priceData.time, longTermXtrender, time),"constant")
const macollongXtrenderCol_interp = interpolate_sparse_series(land_points_onto_series(priceData.time, macollongXtrenderCol, time),"constant")
...
//Land and interpolate on current timeframe
const maShortTermXtrender_interp = interpolate_sparse_series(land_points_onto_series(priceData.time, maShortTermXtrender, time),"constant")
const colShortTermXtrender_interp = interpolate_sparse_series(land_points_onto_series(priceData.time, colShortTermXtrender, time),"constant")
...
if(showColorBooleanLine)
{
  ...
  name:  "Color - Boolean Line (70=Green, -70=Red)",
  color: "white",
  style: 'line',
  thickness: 1});
}
...
const bullSwitch_interp = land_points_onto_series(priceData.time, bullSwitch, time)
const bearSwitch_interp = land_points_onto_series(priceData.time, bearSwitch, time)
debugger;
